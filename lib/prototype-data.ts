...
export const mcpTools: McpToolRecord[] = [
  {
    ...
    status: "启用",
    category: "输入标准化",
    description: "负责把公司名、域名、URL、IP 和 CIDR 统一规范化，生成后续可调度的种子目标。",
    inputMode: "company / domain / url / ip / cidr",
    outputMode: "normalized-targets.json",
    boundary: "平台内部处理",
    ...
    retry: "2 次",
    lastCheck: "今天 11:40",
    notes: "作为首阶段标准化工具，默认自动执行，不直接对目标环境产生外部交互。",
  },
  {
    id: "mcp-05",
    ...
    status: "启用",
    category: "情报扩展",
    description: "聚合 DNS、证书与被动子域情报，补齐项目早期的外网资产视图。",
    inputMode: "domain / company / seed-targets",
    outputMode: "passive-discovery.json",
    boundary: "外部目标交互",
    requiresApproval: false,
  ...
  },
  {
    ...
    status: "禁用",
    category: "网络探测",
    description: "在明确授权范围内做低频端口发现，用于补齐服务识别前的基础网络面。",
    inputMode: "ip / cidr / host-list",
    outputMode: "open-ports.json",
    boundary: "外部目标交互",
    requiresApproval: false,
    endpoint: "mcp://port-scout",
    owner: "MCP 网关",
    ...
    retry: "0 次",
    lastCheck: "昨天 20:35",
    notes: "当前默认关闭，等待后续把更细的端口探测范围规则接入调度器。",
  },
  {
    id: "mcp-02",
    capability: "受控验证类",
    ...
    status: "启用",
    category: "认证验证",
    description: "在审批通过后执行受控认证验证，强调停止条件、痕迹保存和人工可回溯性。",
    inputMode: "entry-url / approval-context / stop-conditions",
    outputMode: "controlled-validation.json",
    boundary: "外部目标交互",
    ...
    retry: "0 次",
    lastCheck: "今天 12:02",
    notes: "默认只有高风险审批通过后才允许执行，且必须绑定证据采集与停止条件。",
  },
  {
    id: "mcp-03",
    capability: "截图与证据采集类",
    ...
    status: "异常",
    category: "证据采集",
    description: "负责页面截图、关键响应片段抓取和结构化证据包生成，是结果与审批链路的核心采证工具。",
    inputMode: "url / response / task-context",
    outputMode: "evidence-bundle.json",
    boundary: "外部目标交互",
    requiresApproval: false,
    endpoint: "mcp://capture-evidence",
    owner: "证据中心",
    ...
    retry: "1 次",
    lastCheck: "今天 12:12",
    notes: "当前处于异常态，导致关键页面截图链路不稳定，需要优先巡检恢复。",
  },
  {
    id: "mcp-07",
    ...
    retry: "1 次",
    lastCheck: "今天 10:54",
    notes: "默认作为低风险页面识别能力使用，配合证据采集与 API 结构发现前置运行。",
  },
  {
    id: "mcp-04",
    capability: "接口识别类",
    ...
    status: "启用",
    category: "接口识别",
    description: "负责发现 GraphQL、OpenAPI、Swagger 等接口结构暴露面，帮助形成待验证候选单。",
    inputMode: "url / api-entry / response-headers",
    outputMode: "api-surface.json",
    boundary: "外部目标交互",
    requiresApproval: false,
    endpoint: "mcp://graphql-surface-check",
    owner: "MCP 网关",
    defaultConcurrency: "2",
    rateLimit: "20 req/min",
    timeout: "60s",
  ...
  },
  {
    ...
    status: "启用",
    category: "结果导出",
    description: "将项目结果、证据摘要和审批记录导出为面向研究员或客户的结构化报告包。",
    inputMode: "project-summary / findings / evidence-links",
    outputMode: "report-package.zip",
    boundary: "平台内部处理",
    requiresApproval: false,
    ...
    retry: "1 次",
    lastCheck: "今天 09:40",
    notes: "只处理平台内部结果聚合与导出，不直接与目标环境交互。",
  },
]
...
export const mcpCapabilityRecords: McpCapabilityRecord[] = [
  {
    ...
    connectedTools: ["seed-normalizer"],
  },
  {
    id: "cap-dns-passive",
    name: "DNS / 子域 / 证书情报类",
  ...
  },
  {
    ...
    connectedTools: ["web-surface-map"],
  },
  {
    id: "cap-api-surface",
    name: "HTTP / API 结构发现类",
    ...
    connectedTools: ["graphql-surface-check"],
  },
  {
    ...
    connectedTools: ["auth-guard-check"],
  },
  {
    ...
    connectedTools: ["capture-evidence"],
  },
  {
    ...
    connectedTools: ["report-exporter"],
  },
]
...
export const mcpRuns: McpRunRecord[] = [
  {
    id: "run-20260326-001",
    projectId: "proj-huayao",
    projectName: "华曜科技匿名外网面梳理",
  ...
  },
  {
    ...
    linkedApprovalId: "APR-20260326-012",
    summaryLines: [
      "夜间窗口未确认，GraphQL schema 暴露验证已延后。",
      "主路径继续推进资产归属与结果整理，不阻塞当前项目结果面。",
    ],
  },
]
