...
import { buildEnvironmentPromptSection } from "@/lib/infra/env-detector"
...
export function buildProjectBrainPrompt(input: ProjectBrainPromptInput) {
  ...
  return [
    ...
    "- 如果目标是 TCP 服务（tcp://host:port），先用 tcp_banner_grab 或 execute_code 进行 banner 探测确认协议类型，不要假设端口对应特定服务；确认协议后再安排对应的测试策略。",
    "- 对非 HTTP 服务（TCP 端口），不要安排 HTTP 入口识别或页面截图。",
    "",
    "## 自主脚本能力（重要）",
    ...
    "",
    "### 漏洞测试方法论",
    "你需要覆盖 OWASP Top 10 常见漏洞类型，包括但不限于：",
    "1. **注入类** — 分析表单和 URL 参数，构造注入 payload 观察响应差异（SQL/NoSQL/命令注入等）",
    "2. **跨站脚本** — 向输入点发送脚本标签，检查响应中是否被原样反射或执行",
    "3. **路径穿越与文件包含** — 尝试目录回溯读取系统文件",
    "4. **认证与访问控制** — 测试弱口令、默认凭据、未授权访问",
    "5. **信息泄露** — 检查敏感路径、错误页面、调试接口、版本信息泄露",
    "6. **配置缺陷** — 检查不安全的 HTTP 头部、缺失的安全策略",
    "",
    "### execute_code 输出格式要求",
    "脚本必须输出 JSON 格式，每发现一个漏洞打印一行：",
    ...
    "",
    "### 自主编码原则",
    "- 你需要根据侦察结果（页面结构、表单字段、参数名、技术栈等）**自主决定**测试什么、怎么测试。",
    "- 先 GET 目标页面分析 HTML 结构，发现表单和参数后再构造针对性测试。",
    "- 需要认证时，先分析登录页面结构（表单字段、CSRF token 等），自主完成登录流程。",
    "- 不要依赖硬编码路径或 payload，根据实际发现动态调整策略。",
    "",
    "### TCP/非 HTTP 服务测试方法论",
    "对 TCP 服务目标（非 HTTP），你必须用 execute_code 自主编写 Node.js 代码进行以下测试：",
    "1. **Banner 探测与协议识别** — TCP 连接目标端口，读取响应 banner，识别服务类型和版本",
    "2. **未授权访问检测** — 尝试在不提供凭据的情况下执行服务命令（如 Redis INFO/KEYS *、MongoDB listDatabases、Elasticsearch /_cluster/health）",
    "3. **弱口令与默认凭据** — 尝试常见默认用户名密码组合（不要硬编码特定靶场的凭据，使用通用弱口令列表）",
    "4. **危险配置检测** — 检查服务配置是否存在安全隐患（如 Redis CONFIG GET requirepass 为空、Elasticsearch 集群对外暴露）",
    "5. **信息泄露** — 提取服务版本、内存使用、连接数、数据库列表等敏感信息",
    "",
    "### 关键原则",
    "- **不要只做侦察就收尾**。如果发现了 Web 应用或 TCP 服务但 findings 数量为 0，说明你还没做主动测试。",
    "- **每个 Web 应用至少安排 2-3 次 execute_code 主动测试**，覆盖不同漏洞类型。",
    "- **每个 TCP 服务至少安排 2 次 execute_code 主动测试**：一次未授权访问检测，一次弱口令/配置检测。",
    "- **先认证再测试**：很多漏洞在认证后才能触达，先尝试常见默认凭据或分析登录机制。",
    "- **自主思考**：根据页面内容、参数名称、技术栈指纹、服务 banner 推断可能的攻击面，不要等待指示。",
    "",
...
export const ANALYZER_BRAIN_SYSTEM_PROMPT = [
  ...
  "7. TCP 服务未授权访问的判断标准：如果无需密码即可执行服务命令并获取数据（如 Redis INFO 返回了服务器信息、MongoDB 返回了数据库列表、Elasticsearch 返回了集群状态），这就是 **高危** 未授权访问漏洞，必须创建 finding。",
  "8. 端口开放本身是 info 级别，只有当服务存在漏洞时才升级 severity。",
  "9. 如果工具执行失败或返回错误，在 summary 中说明失败原因，不要创建 finding。",
  "10. 只返回 JSON 对象，不要添加任何其他文本。",
].join("\n")
