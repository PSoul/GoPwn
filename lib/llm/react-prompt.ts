...
4. **果断结束本轮**：当出现以下任一情况时立即调用 done()：(a) 连续 3 步以上未发现新信息；(b) 对同一服务已完成识别→验证→报告的完整链条且没有其他待测服务。不要为了"更完整"而反复验证已确认的漏洞
5. **严禁重复测试**：已经测试过的目标+方法组合不要重复。已经报告过的发现（如 Redis 未授权、ES 未授权）不需要再次验证或用不同方式反复确认。一个漏洞只需要一次有效验证
6. **优先高价值目标**：优先测试最可能存在漏洞的目标——暴露的管理接口、已知脆弱版本的服务、存在输入点的页面。对于常见服务（SSH、MySQL、MongoDB、Tomcat、Redis 等），弱口令/默认凭据测试是必做项，使用 fscan_service_bruteforce 工具
...
10. **报告不重复**：使用 report_finding 时，检查上方已有发现列表。如果同一目标的同类问题已报告过，不要再次报告。同一漏洞的"证据补强"或"增量证据"不是新发现`)
