...
```
1. 共享方法论基础（从 mcps/pentest-agent-prompt.md 加载，若文件不存在则使用内置 fallback）
2. 角色定义（ReAct Agent：Thought → Action → Observation 迭代循环）
3. 项目信息（项目名、当前阶段/phase label、轮次 round/maxRounds、步骤 stepIndex/maxSteps）
4. 测试目标列表（[type] value 格式）
5. Scope 规则（scopeDescription 或默认约束：仅测授权目标、禁拒绝服务、禁探测第三方）
6. 已发现资产（[kind] value — label 格式，无资产时显示占位符）
7. 已发现漏洞（[severity/status] title → affectedTarget 格式，无发现时显示占位符）
8. 行为准则（7 条：先思考再行动、根据实际结果决策、发现新目标立即测试、
...
每执行 3 步，react-worker 重新从数据库拉取最新资产和发现，调用 `ctxManager.updateSystemPrompt()` 替换 `messages[0]`，无需重建整个消息列表。
