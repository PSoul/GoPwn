...
import { projectDetailSummary, projectTimeline } from "@/lib/prototype-data"
...
export function ProjectStageFlow() {
  return (
    <SectionCard title="阶段流转" eyebrow="Flow Control" description="主阶段不重置，但允许前置阶段因新线索回流并补充任务。">
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="grid gap-3">
          {projectTimeline.map((stage, index) => (
            <div key={stage.title} className="grid gap-3 md:grid-cols-[auto_1fr_auto] md:items-center">
              <div className="flex items-center gap-3">
                ...
                {index < projectTimeline.length - 1 ? <ArrowRight className="hidden h-4 w-4 text-slate-400 md:block" /> : null}
              </div>
        ...
        <div className="space-y-4">
          <div className="rounded-3xl border border-rose-200 bg-rose-50/80 p-5 dark:border-rose-900/80 dark:bg-rose-950/40">
            <p className="text-sm font-semibold text-rose-700 dark:text-rose-200">当前阻塞</p>
            <p className="mt-2 text-sm leading-6 text-rose-800/90 dark:text-rose-100/90">{projectDetailSummary.blockingReason}</p>
          </div>

          <div className="rounded-3xl border border-sky-200 bg-sky-50/80 p-5 dark:border-sky-900/80 dark:bg-sky-950/40">
            <p className="text-sm font-semibold text-sky-700 dark:text-sky-200">下一步建议</p>
            <p className="mt-2 text-sm leading-6 text-sky-800/90 dark:text-sky-100/90">{projectDetailSummary.nextStep}</p>
          </div>
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50/80 p-5 dark:border-amber-900/80 dark:bg-amber-950/40">
            ...
            <p className="text-sm leading-6 text-amber-800/90 dark:text-amber-100/90">{projectDetailSummary.reflowNotice}</p>
          </div>
        </div>
      </div>
    </SectionCard>
