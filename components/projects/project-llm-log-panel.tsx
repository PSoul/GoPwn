        ...
        {items.map((item: Record<string, unknown>, idx: number) => {
          ...
          return (
            <div key={idx} className="rounded-lg border border-slate-200/80 bg-slate-50/50 px-3 py-2 dark:border-slate-700/60 dark:bg-slate-900/30">
              ...
              {!!item.target && (
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">目标: {String(item.target)}</p>
              )}
            </div>
          )
        })}
...
export function ProjectLlmLogPanel({
  ...
  const loadLogs = useCallback(async () => {
    try {
      ...
      const payload = await apiFetch<{ items: LlmCallLog[] }>(`/api/projects/${projectId}/llm-logs${roleParam}`)
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {logs.map((log) => {
            ...
            return (
              <div key={log.id} className="rounded-2xl border border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-950">
                <button
                  type="button"
                ...
                >
                  <div className="flex flex-1 flex-wrap items-center gap-2">
                    <StatusBadge tone={roleTone[log.role] ?? "info"}>{roleLabels[log.role] ?? log.role}</StatusBadge>
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800">
                    ...
                    {log.error && (
                      <div className="border-t border-slate-100 px-5 py-2.5 dark:border-slate-800">
                        <span className="text-xs text-rose-600 dark:text-rose-400">错误: {log.error}</span>
                      </div>
                    )}
                  </div>
                )}
