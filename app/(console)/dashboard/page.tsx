...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[36px] border border-slate-200/80 bg-white p-6 dark:border-slate-800 dark:bg-slate-950">
          ...
          {metrics.map((metric) => {
            ...
            return (
              <Link
                key={metric.label}
                href={meta.href}
                className="group rounded-[28px] border border-slate-200/80 bg-slate-50/85 p-5 transition-colors hover:border-slate-300 hover:bg-slate-100/80 dark:border-slate-800 dark:bg-slate-900/70 dark:hover:border-slate-700 dark:hover:bg-slate-900"
              >
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {systemOverview.map((item) => (
          <Link
            key={item.title}
            href={item.href}
            className="rounded-[28px] border border-slate-200/80 bg-white p-5 transition-colors hover:border-slate-300 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:hover:border-slate-700 dark:hover:bg-slate-900/80"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
            ...
            <div className="mt-5 space-y-3">
              {recentResults.map((item) => (
                <Link
                  key={item.id}
                  href={item.href}
                  className="block rounded-[24px] border border-slate-200/80 bg-slate-50/85 p-4 transition-colors hover:border-slate-300 hover:bg-slate-100/80 dark:border-slate-800 dark:bg-slate-900/70 dark:hover:border-slate-700 dark:hover:bg-slate-900"
                >
                  <div className="flex items-start justify-between gap-3">
