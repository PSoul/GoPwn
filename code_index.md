...
> 最后更新: 2026-04-04 | 项目版本: v0.9.0 (Phase 24 — 概念精简 + 实时仪表盘 + 路由清理)
...
- `/projects/[id]` — **[Phase 24 重设计]** 项目实时仪表盘（`ProjectLiveDashboard`），SSE 驱动实时更新，3 Tab: 漏洞/资产/执行日志
- `/projects/[id]/edit` — 编辑项目
- `/projects/[id]/operations` — 执行控制面板（审批状态 + 调度开关 + 报告导出）
- `/projects/[id]/vuln/[findingId]` — **[Phase 24 新增]** 漏洞详情（含原始输入输出、截图、修复建议）
...
**Phase 24 删除页面**: `/evidence/*`（证据降级为内部概念）、`/approvals`（审批改为项目内联）、`/projects/[id]/results/*`（合并到实时仪表盘 Tab）、`/projects/[id]/ai-logs`（合并到执行日志 Tab）
...
**[Phase 24 新增]** `project-live-dashboard.tsx` — 实时仪表盘主组件，SSE 驱动，3-Tab 布局
**[Phase 24 新增]** `project-stats-bar.tsx` — 四列实时指标卡片（资产/漏洞/高危/执行中）
**[Phase 24 新增]** `project-vuln-tab.tsx` — 漏洞列表 Tab（含状态控制）
**[Phase 24 新增]** `project-asset-tab.tsx` — 资产列表 Tab（含类型/范围 badge）
**[Phase 24 新增]** `project-activity-log.tsx` — 执行日志时间线（彩色标记点）
**[Phase 24 新增]** `project-approval-bar.tsx` — 审批内联通知条（琥珀色可折叠）
**[Phase 24 新增]** `finding-detail.tsx` — 漏洞详情（含原始 I/O、截图、修复建议）
