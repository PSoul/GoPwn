...
> 最后更新: 2026-04-05 | 项目版本: v0.9.2 (feature/react-iterative-execution — ReAct 迭代执行引擎)
...
`function-calling.ts` — OpenAI Function Calling 工具定义转换；将 MCP 工具列表映射为 OpenAI functions 格式，提供 `getControlFunctions()`（finish_round / request_approval 控制函数）
`tool-input-mapper.ts` — 将 LLM function call 参数映射为 MCP 工具输入格式
`call-logger.ts` — LLM 调用日志记录 | `openai-provider.ts` — OpenAI 兼容 LLM 客户端（含 response_format 自动降级）
`provider.ts` — LLM Provider 类型与接口定义 | `system-prompt.ts` — 系统级提示词基础模板

### lib/workers/ — 后台作业 Worker
`react-worker.ts` — **[ReAct 新增]** 处理 `react_round` 作业；运行 ReAct 循环（LLM function call → MCP 工具执行 → 结果回填 → 下一步），支持 finish_round / request_approval 两种终止路径；单轮最多 `MAX_STEPS_PER_ROUND=30` 步，工具超时 5 分钟
