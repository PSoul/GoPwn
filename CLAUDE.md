...
- LLM provider 使用 SSE 流式调用，支持任意 OpenAI 兼容 API 代理
- 项目描述会注入所有 LLM Prompt（ReAct / Analyzer / Reviewer），用于智能 Scope 判断
- fscan MCP server 已适配 v2.0 参数格式（v1.x 的 `-m portscan` 等已移除）
- react-worker 每步记录 MCP 工具执行日志（mcp_result / mcp_error），含耗时和输出预览
